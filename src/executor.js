...
Executor.prototype.initialize = function() {
	...
	api.login()
		...
		.then(function(schema) {
			// IMPORTANT: refParser returns the dereferenced object. Some versions mutate in-place,
			// but assigning here makes behavior deterministic.
		...
		switch(task.type.toLowerCase()) {
	    	...
	    	default: {
				processRequest(key, task, _this, jobLog)
					...
					.catch(function(error) {
						jobLog.error(error.stack ? error.stack : error);
						deferred.reject(error);
					});
				break;
			}
